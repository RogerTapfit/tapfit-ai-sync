...
serve(async (req) => {
  ...
  try {
    ...
    // Generate the avatar image using AI
    const prompt = `A 3D robot ${animalType} fitness coach character, metallic cybernetic body with ${accentColor} accents, glowing ${accentColor} energy core in chest, ${accentColor} LED eyes, sleek ${gender === 'female' ? 'feminine elegant' : 'masculine powerful'} design, futuristic armor plating, standing in heroic confident pose, ${theme} themed details, ${accentColor} gradient cosmic background with particle effects, highly detailed, octane render style, professional 3D art, centered composition. Ultra high resolution.`;
