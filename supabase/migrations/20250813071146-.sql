...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Public can view character images" ON storage.objects FOR SELECT USING (bucket_id = ''character-images'')';
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Admins can manage character images insert" ON storage.objects FOR INSERT WITH CHECK (bucket_id = ''character-images'' AND public.has_role(auth.uid(), ''admin''::public.app_role))';
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Admins can manage character images update" ON storage.objects FOR UPDATE USING (bucket_id = ''character-images'' AND public.has_role(auth.uid(), ''admin''::public.app_role))';
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Admins can manage character images delete" ON storage.objects FOR DELETE USING (bucket_id = ''character-images'' AND public.has_role(auth.uid(), ''admin''::public.app_role))';
  END IF;
END $$;
